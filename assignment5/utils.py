...
def pad_sents_char(sents, char_pad_token):
    ...
    sentence_maxlen = max( sentence_lengths )

    
    for sentence in sents:
        sentence_padded = list()
        sentence_lengths = len(sentence)
        for word in sentence:
            char_counter = 0
            word_length = len( word )
            if word_length < 21:
                to_pad = [ char_pad_token ] * (21 - word_length)
                word = word +  to_pad
            else:
                word = word[0:20]
            sentence_padded.append( word )   
        sentence_to_pad = [[ char_pad_token ] * 21 ] * (sentence_maxlen - sentence_lengths) 
        sentence_padded = sentence_padded  + sentence_to_pad
