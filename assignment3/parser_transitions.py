...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            lastword = self.stack.pop( -1 )
            secondlastword = self.stack.pop( -1 )
            tup = ( lastword, secondlastword )
            self.stack.append( lastword  )
            self.dependencies.append( tup )
        elif transition == 'RA':
            lastword =       self.stack.pop( -1 )
            secondlastword = self.stack.pop( -1 )        
            tup = ( secondlastword, lastword )
...
def minibatch_parse(sentences, model, batch_size):
    ...
    Initialize partial parses as a list of PartialParses, 
    one for each sentence in sentences
    Initialize unfinished parses
    as a shallow copy of partial parses 
    ...
    do 
        ...
        Remove the completed (empty buffer and stack of size 1) parses from unfinishedparses
    end while                                                
    """
    dependencies = []
